...
type PricingPlan = {
  ...
  const getPricingPlans = async () => {
    ...
    if (!baseUrl) return [] as PricingPlan[]

    try {
      const response = await fetch(`${baseUrl}/pricing`, { next: { revalidate: 60 } })
      if (!response.ok) return [] as PricingPlan[]
...
const getPricingPlans = async (): Promise<PricingPlan[]> => {
  try {
    ...
    const response = await fetch(`${apiUrl}/pricing`, { next: { revalidate: 60 } })
    if (!response.ok) {
            ...
            <div key={plan.name} className="bg-bg-secondary border border-border-divider rounded-lg p-6">
              ...
              {plan.paymentUrl ? (
                <a
                  ...
                  target="_blank"
                  rel="noopener noreferrer"
                  className="mt-4 inline-flex items-center gap-2 rounded-lg border border-border-divider bg-brand-red px-4 py-2 text-sm font-semibold text-white hover:bg-brand-red-hover transition-colors"
                >
                  Pay now
                </a>
              ) : null}
